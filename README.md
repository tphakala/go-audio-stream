...
- **Depacketizers**: AAC (RFC 3640 AAC-hbr) and MP4A-LATM (RFC 3016, out-of-band
  ...
  ADPCM (16/24/32/40 kbps, from a `G726-NN` rtpmap (RFC 3551) or an
  `AAL2-G726-NN` one (ITU-T I.366.2 Annex E; the same codewords in the opposite
  bit order), or the static payload type 2), and L16 linear PCM (RFC 3551, from an `L16` rtpmap or the static payload
