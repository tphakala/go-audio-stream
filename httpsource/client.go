...
type Client struct {
	...
	// classifyOpenRead maps a raw read error hit while parsing the format header
	...
	// before the synchronous format probe and read only there; it is nil outside
	// an Open (and in a zero Client), where the parsers report their own format
	// error, preserving the pre-#92 malformed/short-stream behavior.
	classifyOpenRead func(error) error
...
func Open(ctx context.Context, cfg Config) (*Client, error) {
	...
	c.classifyOpenRead = func(err error) error { return classifyHeaderReadErr(ctx, err, &timedOut) }
