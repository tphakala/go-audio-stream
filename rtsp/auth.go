...
var (
	...
	// variant).
	ErrUnsupportedAuth = errors.New("rtsp: unsupported authentication scheme or algorithm")
	// ErrMissingNonce means a Digest challenge carried no nonce.
	ErrMissingNonce = errors.New("rtsp: digest challenge missing nonce")
	// ErrMissingCNonce means a qop-bearing Digest challenge was answered with
	// an empty client nonce.
	ErrMissingCNonce = errors.New("rtsp: digest qop requires a client nonce")
)
...
// and deterministic; M4b generates a random cnonce (crypto/rand) and tracks
// the nonce count.
type DigestInput struct {
	// Method is the request method, for example "DESCRIBE".
	Method string
...
// compile into package rtsp and reach it directly, and the M4b client is in
// the same package, so it need not be part of the public API surface.
